    ...
    [MarshalAs(UnmanagedType.LPWStr)] string pszPropertyPage);
        ...
        private void showSelectProtocolDialog(string protocolToFind) {
            ...
            foreach (var program in registeredPrograms) {
                if (!appnames.ContainsKey(program.DisplayName)) {
                    if (program.Capabilities.Contains(protocolToFind)) {
                        appnames.Add(program.DisplayName, "Program");
                    }
                }
            }
        ...
        private void button1_Click(object sender, EventArgs e) {
            AppAssociation.getProgramProtocolsProgIds("Outlook (classic)", registeredPrograms);
        }
    }
}
