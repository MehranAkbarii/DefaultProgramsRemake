...
namespace FileAssociationLibrary {
    ...
    public static class AppAssociation {
        ...
        public static string getProtocolDiscription(string protocol) {
            string str = @"SOFTWARE\Classes\" + protocol;
        ...
        public static void changeProtocolAssociation(string programName, string protocol , string flag) {
            //use SetUserFTA in backgroound to change association by prog id
            //win32 apps curenntly not soppurted 
            var progs = FileAssociationManager.GetRegisteredApplications();
            var prog = progs.SingleOrDefault(prog => prog.DisplayName == programName);
            var progId = prog.ProgIDs[protocol].ToString();
            //SetUserFTA.exe {protocol} {progid}
            setUserFTA(protocol, progId);
        ...
        private static string setUserFTA(string protocol, string progId) {
            ...
            ProcessStartInfo processInfo = new ProcessStartInfo {
                ...
                Arguments = $"{protocol} {progId}", // Arguments for the process
                CreateNoWindow = true, // Don't show a console window
                UseShellExecute = false, // Don't use the OS shell to start the process
                RedirectStandardOutput = true, // Redirect output to read it in background (optional)
                RedirectStandardError = true, // Redirect error to read it (optional)
            };
        ...
        public static void getProgramProtocolsProgIds(string programName, List<RegisteredApplication> registeredApplications) {
            var progids = registeredApplications.Single(registeredprog => registeredprog.DisplayName == programName).ProgIDs;
            foreach (var progid in progids.Values) {
                MessageBox.Show(progid.ToString());
            }
           
        }


        public static string getProgramsProtocolsByName() {
            return "";
        }

        public static string getUWPappProtoclosProgIdByFamilyname(string familyName, string protocol) {
            //get suporeted apps prog Ids for the protocol from family name 
            //apps prog ids for protocols should readed from registery
            //apps has multiple prog ids for different potocols
            string progid = "";
            string str = @"SOFTWARE\Classes\Local Settings\Software\Microsoft\Windows\CurrentVersion\AppModel\Repository\Packages\";
            List<string> subkeys = Registry.CurrentUser.OpenSubKey(str).GetSubKeyNames().ToList();
            string familyNameSubkey = subkeys.Single(subkey => subkey.Contains(familyName.Split('_')[0]));
            str += familyNameSubkey + @"\App\Capabilities\URLAssociations";
            RegistryKey key = Registry.CurrentUser.OpenSubKey(str);

            if (key != null) {
                progid = key.GetValue(protocol) as string;
            } else {
                str = @"SOFTWARE\Classes\Local Settings\Software\Microsoft\Windows\CurrentVersion\AppModel\Repository\Packages\" + familyNameSubkey;
                List<string> subkeysOfVariations = Registry.CurrentUser.OpenSubKey(str).GetSubKeyNames().ToList();
                foreach (string subkey in subkeysOfVariations) {
                    str = @"SOFTWARE\Classes\Local Settings\Software\Microsoft\Windows\CurrentVersion\AppModel\Repository\Packages\"
                                                                                                         + familyNameSubkey + "\\" + subkey + "\\Capabilities\\URLAssociations";
                    key = Registry.CurrentUser.OpenSubKey(str);
                    if (key != null) {
                        if ((key.GetValue(protocol) as string != "") && (key.GetValue(protocol) as string != null)) {
                            progid = key.GetValue(protocol) as string;
                        }
                    }
                }
            ...
            if (extensionInfo.ContextMenuVerbs.DefaultVerb == null) {
                ...
                if (info != null) {
                    ...
                    if (info.StartsWith("AppX")) {
                        ...
                        str = @"SOFTWARE\Classes\" + info + @"\shell\Open\command";
                        key = Registry.LocalMachine.OpenSubKey(str);
                        if (key != null) {
                            string exepath = key.GetValue("") as string;
                            exepath = FileAssociationManager.CleanExePath(exepath, false);
            ...
            while (true) {
                while (true) {
                    ...
                    if (!string.IsNullOrEmpty(name) || !string.IsNullOrEmpty(str2)) {
                        ...
                        list.Add(item);
                        RegistryKey keyProtocol = key2.OpenSubKey("URLAssociations");
                        if (keyProtocol != null) {
                            item.Capabilities.AddRange(keyProtocol.GetValueNames());
                            foreach (string strprotocol in item.Capabilities) {
                                item.ProgIDs.Add(strprotocol , keyProtocol.GetValue(strprotocol));
                            }
                        }
                        ...
                        if (key4 != null) {
                            ...
                            foreach (string str3 in item.Capabilities) {
                                if (!item.ProgIDs.ContainsKey(str3)) {
